...
#         if(total_quantity == 0):
#             messages.info(request,'Select product for place order.')
...
def get_order(request):
    if request.method == 'POST':
        ...
        num3 = int(request.POST.get('number3', 0))
        ph_num = request.user.phone
        username = request.user.username
        
        items = item.objects.all()
        if len(items) >= 3:
            name_1 = items[0].item_name
            name_2 = items[1].item_name
            name_3 = items[2].item_name
            
            price_1 = int(items[0].item_price)
            price_2 = int(items[1].item_price)
            price_3 = int(items[2].item_price)
            ...
            messages.error(request, 'Not enough items available to process order.')
            return redirect('order')
    else:
        return render(request, "order.html")
